...
/**
 * KDTree is a class supporting KD-tree insertion, deletion, equality search, range search, and nearest neighbor(s) using double-precision
 * floating-point keys. Splitting dimension is chosen naively, by depth modulo K. Semantics are as follows:
 * 
 * <UL>
 * <LI>Two different keys containing identical numbers should retrieve the same value from a given KD-tree. Therefore keys are cloned when a node is
 * inserted. <BR>
 * <BR>
 * <LI>As with Hashtables, values inserted into a KD-tree are <I>not</I> cloned. Modifying a value between insertion and retrieval will therefore
...
public class KDTree {
  ...
  public void insert(double[] key, Object value) {
    ...
    if (key.length != kdimensions) {
      throw new RuntimeException("KDTree: wrong key size!");
    } else {
      rootNode = KDNode.ins(new HPoint(key), value, rootNode, 0, kdimensions);
    }
  ...
  public Object search(double[] key) {
    ...
    KDNode kd = KDNode.srch(new HPoint(key), rootNode, kdimensions);

    return (kd == null ? null : kd.value);
  }

  /**
   * Delete a node from a KD-tree. Instead of actually deleting node and rebuilding tree, marks node as deleted. Hence, it is up to the caller to
  ...
  public void delete(double[] key) {
    ...
    if (key.length != kdimensions) {
      ...
      KDNode t = KDNode.srch(new HPoint(key), rootNode, kdimensions);
      if (t == null) {
  ...
  /**
   * Find KD-tree nodes whose keys are <I>n</I> nearest neighbors to key. Uses algorithm above. Neighbors are returned in ascending order of distance
   * to key.
   * 
   * @param key
   *          key for KD-tree node
  ...
  public Object[] nearest(double[] key, int n) {
    ...
    if (n < 0 || n > nodeCount) {
      throw new IllegalArgumentException(
          "Number of neighbors (" + n + ") cannot" + " be negative or greater than number of nodes (" + nodeCount + ").");
    }
    ...
    KDNode.nnbr(rootNode, keyp, hr, maxDistSqd, 0, kdimensions, nnl);
  ...
  public Object[] range(double[] lowk, double[] uppk) {
    ...
    if (lowk.length != uppk.length) {
      throw new RuntimeException("KDTree: wrong key size!");
    } else if (lowk.length != kdimensions) {
      throw new RuntimeException("KDTree: wrong key size!");
    } else {
      Vector<KDNode> v = new Vector<KDNode>();
      KDNode.rsearch(new HPoint(lowk), new HPoint(uppk), rootNode, 0, kdimensions, v);
